...
export async function generateMetadata({
  ...
  return {
    ...
    description: `${name}의 질환별 효과, 근거 수준, 제형과 안전 정보를 확인합니다.`,
...
export default async function SubstancePage({
  ...
  return (
    <main id="main-content" className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-14">
      ...
      <header className="mt-7 border-b border-slate-200 pb-8">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-md bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">
            {detail?.substance.substance_type === 'drug' || local?.type === '의약품'
              ? '의약품'
              : '영양제'}
          </span>
      ...
      {isSupplement && (
        <section className="mt-8 rounded-lg border border-amber-200 bg-white p-5 sm:p-6" aria-labelledby="coupang-link-heading">
          <p className="text-sm font-bold text-amber-700">관련 상품 확인</p>
          <h2 id="coupang-link-heading" className="mt-2 text-xl font-bold text-slate-950">{name} 관련 상품을 확인해 보세요</h2>
          <p className="mt-2 text-sm leading-7 text-slate-600">쿠팡 상품 검색 API에서 현재 판매 중인 관련 상품을 최대 10개까지 확인합니다.</p>
          <Link href={coupangSearchUrl(coupangKeyword, name)} className="mt-5 inline-flex min-h-12 items-center justify-center rounded-lg bg-amber-400 px-5 text-base font-bold text-amber-950 transition hover:bg-amber-500">
      ...
      <section className="mt-8 rounded-lg border border-emerald-200 bg-emerald-50/70 p-6">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-700">
          같은 성분이라도 질환, 원료, 용량, 제형에 따라 연구 결과가 달라질 수 있습니다. 아래 질환별 결과에서 효과 크기와 근거 수준을 함께 확인하세요.
        </p>
      </section>
      <section className="mt-12" aria-labelledby="condition-evidence">
        ...
        {conditions.length > 0 ? (
          ...
          <div className="surface-card mt-5 p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">성분 설명은 준비되어 있지만 질환별 비교 데이터는 추가 검수 중입니다.</p>
          </div>
        )}
      </section>
      <aside className="mt-12 border-t border-slate-200 pt-8">
        <p className="text-sm leading-7 text-slate-500">
          처방약을 임의로 변경하거나 중단하지 마세요. 영양제도 질환과 복용약에 따라 주의가 필요합니다.
        </p>
