...
export async function generateMetadata({ searchParams }: ConditionsPageProps): Promise<Metadata> {
  ...
  return {
    title: query ? `${query} 관련 질환 검색 결과` : '질환별 약·영양제 근거 비교',
    description: query
      ? `${query}와 관련된 질환을 찾아 약과 영양제의 효과 및 근거 수준을 비교합니다.`
      : '골관절염, 불면증, 고혈압, 우울, 불안 등 건강 문제별 약과 영양제를 효과 크기와 근거 수준으로 비교합니다.',
  ...
  return (
    <main id="main-content" className="content-shell py-10 sm:py-16">
      ...
      <header className="max-w-3xl">
        <p className="eyebrow">증상·질환으로 찾기</p>
        <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-5xl">
          내 증상과 가까운 건강 문제를 찾아보세요
        </h1>
        <p className="mt-5 text-lg leading-8 text-slate-700">
          증상이나 질환명을 입력하면 데이터베이스에서 관련 항목을 찾아드립니다. 선택한 뒤 약과 영양제의 실제 연구 결과를 비교할 수 있습니다.
        </p>
      </header>
      ...
      {query ? (
        <section className="mt-10" aria-labelledby="search-results-heading" aria-live="polite">
          <div className="flex flex-col gap-3 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm font-bold text-emerald-700">데이터베이스 검색 결과</p>
          ...
          {conditions.length > 0 ? (
            ...
            <div className="surface-card mt-6 max-w-3xl p-6 sm:p-8">
              <h3 className="text-xl font-bold text-slate-950">검색어를 조금 짧게 바꿔보세요</h3>
              <p className="mt-3 text-base leading-7 text-slate-700">
                증상을 한두 단어로 입력하면 더 잘 찾을 수 있습니다. 예를 들어 ‘잠이 자주 깨요’는 ‘잠’, ‘무릎이 아파요’는 ‘관절’로 검색해 보세요.
              </p>
            </div>
          )}
        </section>
        ...
        <>
          <nav className="mt-8 flex flex-wrap gap-2" aria-label="질환 분류 바로가기">
            {groups.map(([group]) => (
              <a key={group} href={`#${encodeURIComponent(group)}`} className="interactive-link rounded-full border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-600 hover:border-emerald-300">
                {group}
              </a>
            ))}
          </nav>
